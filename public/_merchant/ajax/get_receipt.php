...
elseif((int)$kengen == 99){
    ...
    //やり残しチェック
    $sql = "SELECT * FROM receipts where mk_user_id = :mk_id AND CAST(status AS SIGNED) >= 1 AND CAST(status AS SIGNED) <= 39 ORDER BY user_id,created_at LIMIT :count";
    $stmt = $PDO -> prepare($sql);
    ...
    if($count != $maxCount){
        //まずは割り振り
        $sql = "UPDATE receipts SET mk_user_id = :mk_id where mk_user_id IS NULL AND CAST(status AS SIGNED) >= 1 AND CAST(status AS SIGNED) <= 39 ORDER BY user_id,created_at LIMIT :count"; 
        $stmt = $PDO -> prepare($sql);
        $stmt->bindValue(':mk_id', $mk_id, PDO::PARAM_STR);
...
// テスト権限8ここまで
    ...
    while($row = $stmt->fetch(PDO::FETCH_ASSOC)){    
        ...
        $user[] = array(
            ...
            "status_oil" => $row["status_oil"],
            "status_input" => $row["status_input"],
            "status_diff" => $row["status_diff"],
            "status_term" => $row["status_term"],
            "status_shop" => $row["status_shop"],
            "status_double" => $row["status_double"],
            "status_ngword" => $row["status_ngword"],
