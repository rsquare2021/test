...
@section('content')
    ...
    @else
        <h2 class="mypage_tit mb-0">景品一覧</h2>
    @endif
    
    <div class="user_wrap">
        <div class="products recommend">
            <h3 class="mt-2 mb-4">おすすめの景品</h3>
            <ul class="swiper-wrapper">
                @foreach ($recommends as $recommend)
                    <li class="swiper-slide">
                        ...
                        @endif
                            <div class="img"><img src="https://re-nt-upload.s3.ap-northeast-1.amazonaws.com/products/{{$campaign->id}}/{{ $recommend->product->operation_id }}.jpg"></div>
                            <h4>{{ $recommend->getProductName() }}</h4>
                            <div class="use_point">必要ポイント数：<span>@if($recommend->access_token !== null) @php $giftee_point = $recommend->getPoint() * 2; echo $giftee_point; @endphp @else {{ $recommend->getPoint() }} @endif</span>pts</div>
        ...
        <div class="product_search mt-3 mb-4 pt-3">
            ...
            @endif
                ...
                <input type="submit" class="blue_btn mt-4" value="検索">
            </form>
        ...
        <ul class="lineup products">
            @foreach ($gifts as $gift)
                <li>
                    ...
                    @endif
                        <div class="img"><img src="https://re-nt-upload.s3.ap-northeast-1.amazonaws.com/products/{{$campaign->id}}/{{ $gift->product->operation_id }}.jpg"></div>
                        <h4>{{ $gift->getProductName() }}</h4>
                        <div class="use_point">必要ポイント数：<span>@if($gift->access_token !== null) @php $giftee_point = $gift->getPoint() * 2; echo $giftee_point; @endphp @else {{ $gift->getPoint() }} @endif</span>pts</div>
        ...
        <div class="notes">
            <ul>
                <li>※本キャンペーンに関して応募者が被った損害または損失などについては、当社の故意または重過失に起因する場合を除き、当社は一切の責任を負わないものとします。</li>
